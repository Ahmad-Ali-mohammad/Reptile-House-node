...
const sidebarConfig = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "الخدمات", page: 'services' as DashboardPage, icon: <TagIcon className="w-5 h-5" /> },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "إدارة محتوى الصفحات", page: 'page_content' as DashboardPage, icon: <DocumentIcon className="w-5 h-5" /> },
      { name: "لوحة تحسين محركات البحث", page: 'seo' as DashboardPage, icon: <DocumentIcon className="w-5 h-5" /> }
    ]
  },
  {
    name: "المبيعات",
...
const Sidebar: React.FC<SidebarProps> = ({ activePage, setActivePage, setAppMode, setPage, ordersBadgeCount = 0 }) => {
  ...
  return (
    <aside className="flex h-full w-[min(20rem,calc(100vw-1rem))] flex-col overflow-hidden border-l border-white/10 bg-gray-900/60 shadow-2xl backdrop-blur-2xl sm:w-72">
      <div className="border-b border-white/5 p-6 sm:p-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-amber-500 font-bold uppercase tracking-widest mt-1">بيت الزواحف</p>
          </div>
        </div>
      </div>

      <nav className="scrollbar-hide flex-1 min-h-0 space-y-5 overflow-y-auto p-5 pb-8 text-right sm:space-y-6 sm:p-6 sm:pb-10">
            ...
            <div className={`space-y-1 overflow-hidden transition-all duration-300 ${openSections.includes(section.name) ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
              {section.items.map(item => (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-2xl p-3 text-sm transition-all sm:p-3.5 ${activePage === item.page ? 'bg-amber-500 text-gray-900 font-bold shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}
                >
                  {item.icon}
      ...
      <div className="shrink-0 border-t border-white/5 bg-gray-950/80 p-5 backdrop-blur-xl sm:p-6">
